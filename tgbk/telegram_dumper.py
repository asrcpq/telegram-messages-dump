...
from tgbk.exporter_context import ExporterContext
...
class TelegramDumper(TelegramClient):
	...
	def run(self):
		...
		try:
			...
			print("Received a user's request to interrupt, stopping…")
			ret_code = 1
		finally:
		...
		if self.settings.is_clean:
			try:
				...
				print("Failed to logout and clean session data.")

		print(f"wrote {self.output_total_count} msg")
		return ret_code
	...
	def _getChannel(self):
		...
		if name.startswith('@'):
			...
			try:
				...
				if peer.chats is not None and peer.chats:
					print(f"Chat name {name} resolved into channel {peer.chats[0].id}")
					return peer.chats[0]
				if peer.users is not None and peer.users:
					print('User name {name} resolved into channel {peer.users[0].id}')
					return peer.users[0]
		...
		for dialog in dialogs:
			if dialog.name == name:
				print('Dialog title "{}" resolved into channel id={}'.format(
					name, dialog.entity.id))
				return dialog.entity
			if hasattr(dialog.entity, 'username') and dialog.entity.username == name:
				print('Dialog username "{}" resolved into channel id={}'.format(
					name, dialog.entity.id))
				return dialog.entity
		...
		for _ in range(0, 5):
			try:
				...
				if messages.total > 0 and messages:
					print('Processing messages with ids {}-{} ...'
						   .format(messages[0].id, messages[-1].id))
			except FloodWaitError as ex:
				print('FloodWaitError detected. Sleep for {} sec before reconnecting! \n'
					   .format(ex.seconds))
				sleep(ex.seconds)
				self._init_connect()
				continue
	...
	def _do_dump(self, peer):
		...
		try:
			...
			print('Fetching messages from server failed. ' + str(ex))
			print('Warn: The resulting file will contain partial/incomplete data.')

		# Write all chunks into resulting file
		print('Merging results into an output file.')
		try:
	...
	def _check_preconditions(self):
		...
		if self.settings.is_incremental_mode:
			...
			print('Dumping messages newer than {} using "{}" dumper.'
			...
			print('Dumping {} messages into "{}" file ...'
